...
One step was to filter for a single year instead of working with data over time. We chose the year 2001, because of the other variables, location.gdp was of interest, and 2001 is the most recent year for which there is data on GDP across the world (there is no GDP data for 2014). The variable wealth.worth.in.billions was also renamed to just wealth.
...
inher <- linear_reg() |>
  set_engine("lm") |>
  fit(wealth.worth.in.billions ~ age * industry, data = yes_inherited) 
...
notinher <- linear_reg() |>
  set_engine("lm") |>
  fit(wealth.worth.in.billions ~ age * industry, data = not_inherited) 
...
billion2001 |>
  ggplot(aes(x = wealth.worth.in.billions, fill = industry)) +
  geom_histogram() +
  facet_wrap(~inherited, ncol = 1) +
...
yes_inherited |>
  group_by(industry) |>
  summarize(mean_wealth = mean(wealth.worth.in.billions),
            count = n(),
