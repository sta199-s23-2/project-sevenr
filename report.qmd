...
Including country GDP in these models may have increased the $R^2$ values, but it decreased the adjusted $R^2$ values and increased AIC, suggesting that country GDP was not a valuable explanatory variable to add to the model.
...
yes_inherited |>
  group_by(industry) |>
  summarize(inherited = "inherited",
            mean_wealth = mean(wealth.worth.in.billions),
            count = n())

not_inherited |>
  group_by(industry) |>
  summarize(inherited = "not inherited",
            mean_wealth = mean(wealth.worth.in.billions),
            count = n())
